...
🎙 / 🔴 12m03s / 💬 2/5  ← 待机 / 录音中(显示时长) / 转译进度
  ...
  录音设备 ▸           🎙 麦克风 / 🔊 系统声音(扬声器)，可刷新
  ──────
  设置 ▸              API Key… / Base URL… / 云端模型名… / 设置菜单栏图标… / 打开配置 / 查看日志
  打开录音文件夹
  ──────
  退出 vrec
```
...
- **转译进度**：转译时菜单栏显示 `💬`；长音频(云端)显示分段进度如 `2/5`，本地引擎显示百分比。
- **自定义图标**：「设置 → 设置菜单栏图标…」选一张图片（如你的猫咪照片），会自动裁成方形小图标。菜单栏图标很小(约 18px)，建议选清晰、主体居中的图。
